      ...
      {preview && (
        <div className="mt-3 flex gap-4 items-start">
          ...
          <div className="text-sm space-y-1">
            {status === "reading" && <p className="text-ink/60">Reading the photo with on-device OCR…</p>}
            {status === "done" && (
              <>
                <p>
                  <span className="text-ink/60">OCR reading:</span>{" "}
                  <span className="font-mono">{ocrText || "(no text detected)"}</span>
                </p>
            ...
            {status === "error" && <p className="text-danger">Could not read text from this photo.</p>}
          </div>
        </div>
      )}
    </div>
